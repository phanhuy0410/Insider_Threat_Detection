...
def compute_fuzzy_init_numpy(X, fuzzy_indices, num_rules=3):
    ...
    axes = tuple(range(len(X_fuz.shape) - 1))
    mean = np.mean(X_fuz, axis=axes)
    std = np.std(X_fuz, axis=axes)
    std = np.maximum(std, 1e-3)

    if num_rules == 3: centers_init = np.stack([mean - std, mean, mean + std], axis=-1)
    elif num_rules == 2: centers_init = np.stack([mean - 0.5 * std, mean + 0.5 * std], axis=-1)
    else:
        c_list = [(mean + (r - (num_rules - 1) / 2.0) * std) for r in range(num_rules)]
        centers_init = np.stack(c_list, axis=-1)

    sigmas_init = np.repeat(np.expand_dims(std, -1), num_rules, axis=-1)
    return centers_init.astype(np.float32), sigmas_init.astype(np.float32)
...
class FuzzyMembershipLayer(Layer):
    ...
    def build(self, input_shape):
        if self.fuzzy_indices is not None and len(self.fuzzy_indices) > 0: self.fuz_dim = len(self.fuzzy_indices)
        else: self.fuz_dim = input_shape[-1]
            
        self.centers = self.add_weight(name='fuzzy_centers', shape=(self.fuz_dim, self.num_rules),
                                       initializer=tf.keras.initializers.Constant(self.centers_init), trainable=True)
        self.sigmas = self.add_weight(name='fuzzy_sigmas', shape=(self.fuz_dim, self.num_rules),
                                      initializer=tf.keras.initializers.Constant(self.sigmas_init), trainable=True)
    ...
    def call(self, inputs, training=None):
        ...
        safe_sigmas = tf.nn.softplus(self.sigmas) + 1e-5 
        c = self.centers
        s = safe_sigmas
        ...
        if len(inputs.shape) == 3: new_shape = [input_shape_tf[0], input_shape_tf[1], self.fuz_dim * self.num_rules]
        else:                      new_shape = [input_shape_tf[0], self.fuz_dim * self.num_rules]
            
        reshaped = tf.reshape(memberships, new_shape)
        if self.concat_original: return tf.concat([inputs, reshaped], axis=-1)
        return reshaped
...
def build_session_encoder(T_sess, F_sess, flat_fuzzy_indices, centers_init, sigmas_init, learning_rate=0.0001):
    ...
    x = FuzzyMembershipLayer(centers_init=centers_init, sigmas_init=sigmas_init, 
                             fuzzy_indices=flat_fuzzy_indices, concat_original=False, name='Session_Fuzzy')(inp)
    fuzzy_dim = len(flat_fuzzy_indices) * 3
    x = Dense(fuzzy_dim, activation='relu', name='Session_Rule_Inference')(x)
    ...
    out = GaussianClassOutputLayer(num_classes=4, init_sigma=1.0, name='Session_Fuzzy_Output')(latent_bn)    
    model = Model(inputs=inp, outputs=out, name="Session_Encoder")
    model.compile(optimizer=AdamW(learning_rate=0.0001, weight_decay=1e-4), 
                  loss=CategoricalFocalCrossentropy(gamma=2.0, from_logits=True), 
                  metrics=['accuracy'])
    return model
...
def build_user_encoder(F_user, flat_fuzzy_indices, centers_init, sigmas_init, learning_rate=0.0001):
    ...
    x = FuzzyMembershipLayer(centers_init=centers_init, sigmas_init=sigmas_init, 
                             fuzzy_indices=flat_fuzzy_indices, concat_original=False, name='User_Fuzzy')(inp)    
    fuzzy_dim = len(flat_fuzzy_indices) * 3
    x = Dense(fuzzy_dim, activation='relu', name='User_Rule_Inference')(x)
    x = LayerNormalization(epsilon=1e-6)(x)
    x = Dense(32, activation='relu', name='User_Projection')(x)
    x = Reshape((-1, 1))(x)
    x = LayerNormalization(epsilon=1e-6, name='User_Norm')(x)
    x = FeatureTokenEmbedding(d_model=64, name='User_Feature_Tokens')(x)
    T_user = int(x.shape[1])
    x = saint_block(x, T_dim=T_user, F_feat=64, num_heads=4, key_dim=16, ff_dim=128, dropout=0.2, name_prefix="User_SAINT_1")
    x = GatedTemporalAttention(name='User_Attention')(x)
    latent = Dense(16, activation='relu', name='User_Latent')(x)
    latent_bn = BatchNormalization(name='User_Latent_BN')(latent) 
    out = GaussianClassOutputLayer(num_classes=4, init_sigma=1.0, name='User_Fuzzy_Output')(latent_bn)
    model = Model(inputs=inp, outputs=out, name="User_Encoder")
    model.compile(optimizer=AdamW(learning_rate=0.0001, weight_decay=1e-4), 
                  loss=CategoricalFocalCrossentropy(gamma=2.0, from_logits=True), 
                  metrics=['accuracy'])
    return model
...
def build_score_level_fusion(fused_dim=8, learning_rate=0.0001):
     ...
     x = Dense(fused_dim * 2, activation='relu', name='Cross_Correlation_Expand')(inp)
     x = BatchNormalization()(x)
     x = Dropout(0.2)(x)
     x = Dense(fused_dim, activation='relu', name='Cross_Correlation_Compress')(x)
     x_added = add([inp, x], name='Residual_Score_Add')
     x_final = Dense(16, activation='relu', name='Final_Hidden_Layer', kernel_regularizer=l2(1e-2))(x_added)
     ...
     model = Model(inputs=inp, outputs=out, name="Full_Fuzzy_Fusion_Model")
     model.compile(optimizer=AdamW(learning_rate=0.0001, weight_decay=1e-4), 
                   loss=CategoricalFocalCrossentropy(gamma=2.0, from_logits=True), 
                   metrics=['accuracy'])
     return model
...
if __name__ == '__main__':
    ...
    plot_confusion_matrix_percent(conf_matrix_r52, classes_names, title="Ma Trận Nhầm Lẫn Test R5.2 - SAINT")
    ...
    def plot_training_history_average(histories):
        ...
        axes[0].plot(avg_history['loss'], label='Train Loss', color='blue', linewidth=1.5)
        axes[0].plot(avg_history['val_loss'], label='Validation Loss', color='orange', linewidth=1.5)
        axes[0].set_xlabel('Epochs')
        axes[0].set_ylabel('Loss')
        axes[0].set_title('Loss Curve - SAINT', fontweight='bold')
        axes[0].legend()
        axes[0].grid(True, linestyle=':', alpha=0.6)
        
        if 'accuracy' in avg_history:
            axes[1].plot(avg_history['accuracy'], label='Train Accuracy', color='blue', linewidth=1.5)
            axes[1].plot(avg_history['val_accuracy'], label='Validation Accuracy', color='orange', linewidth=1.5)
            axes[1].set_xlabel('Epochs')
            axes[1].set_ylabel('Accuracy')
            axes[1].set_title('Accuracy Curve - SAINT', fontweight='bold')
            axes[1].set_ylim(0.55, 1.01)
            axes[1].set_yticks(np.arange(0.55, 1.01, 0.1))
            axes[1].set_ylim(bottom=0.54)
            axes[1].legend()
    ...
    plot_multiclass_roc(y_r52, r52_probs_accum, 4, classes_names, title="Biểu đồ ROC Test R5.2 - SAINT")
