...
def build_session_encoder(T_sess, F_sess, learning_rate=0.00001):
...
def build_user_encoder(F_user, learning_rate=0.0001):
    ...
    x = saint_block(x, T_dim=F_user, F_feat=x.shape[-1], num_heads=2, key_dim=16, ff_dim=64, dropout=0.2, name_prefix="User_SAINT_1")
...
def train_single_fold_ablation(fold_idx, train_weeks, val_weeks, 
    ...
    # ---------------------------------------------------------
    # 🎯 CHỈ INFERENCE TẬP TEST R5.2 (LỌC BỎ KỊCH BẢN 4 + OFFSET ID)
    # ---------------------------------------------------------
...
if __name__ == '__main__':
    ...
    print("\n" + "="*75)
    print("🚀 PIPELINE 4 (DL): TRAIN MIX | VAL MIX (R4.2+R5.2) | TEST CHỈ R5.2")
    print("="*75)
    ...
    TMP_RESULT_DIR = "./temp_results_dl_p4"
    ...
    # ==============================================================================
    OFFLINE_SAVE_PATH = '/kaggle/working/pipeline4_dl_valmix_test_r52_predictions.npz'
    np.savez_compressed(
        OFFLINE_SAVE_PATH,
    ...
    plot_confusion_matrix_percent(conf_matrix_r52, classes_names, title="Ma Trận Nhầm Lẫn Test R5.2 (Ngưỡng Val Mix)")
