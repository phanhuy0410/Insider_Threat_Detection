...
PATH_USERDAY_R52 = f"{TENSOR_DIR_R52}/userday_clean_test_r52.parquet"
...
# ==============================================================================
# 🔄 KHÔI PHỤC USER MAPPING CHO R4.2 VÀ R5.2
# ==============================================================================
...
def build_user_encoder(F_user, learning_rate=0.0001):
    ...
    x = tcn_residual_block(x, n_filters=32, kernel_size=2, dilation_rate=1, dropout_rate=0.2, block_name="User_TCN_D1")
...
# ==============================================================================
# 🌀 PIPELINE HUẤN LUYỆN: TIME-AWARE K-FOLD KẾT HỢP INFERENCE R5.2
# ==============================================================================
def train_single_fold(fold_idx, train_weeks, val_weeks, X_dev_sess_full, y_dev_full, uid_dev_full, w_dev_full, 
                      user_lookup_dev, F_user, 
                      user_lookup_r52, chunk_files_r52): 
    ...
    # --- HUẤN LUYỆN NEURAL NETWORK CLASSIFIER ---
    ...
    print(f" 🔥 MACRO AUC FOLD {fold_idx + 1}: {fold_auc:.4f}")

    # ---------------------------------------------------------
    #INFERENCE TẬP R5.2 (LỌC BỎ KỊCH BẢN 4)
    # ---------------------------------------------------------
    prob_r52_sess_list, prob_r52_user_list = [], []
    for f in chunk_files_r52: 
        with np.load(f) as data:
            if len(data['y']) == 0: continue
        ...
        batch_user_raw = np.array([user_lookup_r52.get(u, np.zeros(F_user)) for u in batch_uid], dtype=np.float32)
...
if __name__ == '__main__':
    ...
    # 2. NẠP DỮ LIỆU R5.2
    print("📥 Nạp dữ liệu R5.2 để kiểm chéo...")
    user_lookup_r52, _ = load_userday_lookup(PATH_USERDAY_R52, user_mapping_r52)
    _, y_r52_full, _, _, _, _, chunk_files_r52 = load_pass1_session(TENSOR_DIR_R52, "test_r52")

    y_r52 = y_r52_full[y_r52_full != 4]
    print(f" -> Đã lọc bỏ Scen 4. Số lượng mẫu R5.2 còn lại tham gia test: {len(y_r52)} mẫu.")
    ...
    for fold_idx, fold_cfg in enumerate(time_folds):
        ...
        p = mp.Process(
            ...
            args=(fold_idx, train_weeks, val_weeks, X_dev_sess_full, y_dev_full, uid_dev_full, w_dev_full, 
                  user_lookup_dev, F_user, 
                  user_lookup_r52, chunk_files_r52)
        )
        p.start()
    ...
    r52_probs_accum = np.zeros((len(y_r52), 4)) # Form ma trận tự động khớp với y_r52 đã lọc
    ...
    print(f"🏆 TRUNG BÌNH MACRO AUC SAU {len(fold_aucs)} FOLD (R4.2 VAL): {np.mean(fold_aucs):.4f}")
    oof_probs_all = np.vstack(oof_probs_list)
    oof_y_all = np.concatenate(oof_y_list)

    # ==============================================================================
    # 💾 TÍNH TOÁN NGƯỠNG TỪ OOF VAL R4.2 (CHUẨN BỊ CHO TEST R5.2)
    # ==============================================================================
    ...
    # KIỂM CHÉO R5.2
    # ==============================================================================
    print("KẾT QUẢ KIỂM CHÉO TRÊN TẬP DỮ LIỆU R5.2")
    ...
    print(f"\n🚀 MACRO AUC TẬP KIỂM CHÉO (R5.2 ĐÃ BỎ SCEN 4): {r52_auc:.4f} 🚀\n")
    ...
    print("\n -> Đang xuất biểu đồ Confusion Matrix cho R5.2...")
    plot_confusion_matrix_percent(conf_matrix_r52, classes_names, title="Ma Trận Nhầm Lẫn R5.2 Kiểm Chéo (Đã bỏ Scen 4)")
    ...
    plot_multiclass_roc(y_r52, r52_probs_accum, 4, classes_names, title="Biểu đồ ROC Đa lớp Kiểm Chéo (R5.2 Đã bỏ Scen 4)")