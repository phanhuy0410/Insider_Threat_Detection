...
def build_full_expert_t2(T_sess, F_sess, F_user, learning_rate=0.0001):
    ...
    positions = tf.range(T_sess)
    pos_emb = Embedding(input_dim=T_sess, output_dim=128, name="Session_PosEmb")(positions)
    x_sess = x_sess + tf.expand_dims(pos_emb, axis=0)
    x_sess = saint_block(x_sess, T_dim=T_sess, F_feat=128, num_heads=4, key_dim=32, ff_dim=256, dropout=0.2, name_prefix="Sess_SAINT_1")
    ...
    x_user = FeatureTokenEmbedding(d_model=32, name='User_Feature_Tokens')(x_user)
    T_user = int(x_user.shape[1])
    x_user = saint_block(x_user, T_dim=T_user, F_feat=32, num_heads=4, key_dim=8, ff_dim=64, dropout=0.2, name_prefix="User_SAINT_1")
...
def train_single_fold_ablation(fold_idx, train_weeks, val_weeks, 
    ...
    # ==============================================================================
    # 🕵️ CHỌN MẪU TRAIN TẦNG 2: HARD BENIGN (80%) + EASY BENIGN (20%) + TOÀN BỘ SCEN2
    # ==============================================================================
    ...
    # 1. Tìm tập Candidate (Hard Benign)
    ...
    if len(idx_benign_easy_pool) >= NUM_EASY:
        ...
        # Nếu ít quá thì bốc random đại từ những mẫu Benign còn lại
...
if __name__ == '__main__':
    ...
    plot_confusion_matrix_percent(conf_matrix_r52, classes_names, title="Ma Trận Nhầm Lẫn Test R5.2 - SAINT 2 Stage")
