...
def build_user_encoder(F_user, learning_rate=0.0001):
    ...
    x = saint_block(x, T_dim=F_user, F_feat=x.shape[-1], num_heads=2, key_dim=8, ff_dim=64, dropout=0.2, name_prefix="User_SAINT_1")
...
# ==============================================================================
# 🌀 PIPELINE HUẤN LUYỆN: TRAIN MIX | VAL MIX | TEST R4.2
# ==============================================================================
def train_single_fold_ablation(fold_idx, train_weeks, val_weeks, 
    ...
    # 2. 🔥 BÍ QUYẾT: Tạo tập Val TỪ TẬP MIX (R4.2 + R5.2)
    ...
    # 🎯 X_val_sess được lấy TỪ TẬP MIX (đã kết hợp R4.2 + R5.2)
    ...
    # --- HUẤN LUYỆN ENCODERS ---
    session_encoder = build_session_encoder(T_sess, F_sess, learning_rate=0.00001)
...
if __name__ == '__main__':
    ...
    print("\n" + "="*75)
    print("🚀 PIPELINE 3 (DL): TRAIN MIX | VAL MIX (R4.2+R5.2) | TEST CHỈ R4.2")
    print("="*75)
    ...
    TMP_RESULT_DIR = "./temp_results_dl_p3"
    ...
    # ==============================================================================
    OFFLINE_SAVE_PATH = '/kaggle/working/pipeline3_dl_valmix_predictions_for_tuning.npz'
    np.savez_compressed(
        OFFLINE_SAVE_PATH,
    ...
    THRESH_S2 = np.percentile(oof_benign_probs[:, 2], 99.3)  
    THRESH_S3 = np.percentile(oof_benign_probs[:, 3], 99.97)
    ...
    plot_confusion_matrix_percent(conf_matrix_test, classes_names, title="Ma Trận Nhầm Lẫn Test R4.2")
    ...
    plot_multiclass_roc(y_test, test_probs_accum, 4, classes_names, title="Biểu đồ ROC Test R4.2")