...
TENSOR_DIR_R52_218 = "/kaggle/input/datasets/phanthanhhoang/r4-2-r5-2-218-features/tensor_data_r52_218"
...
# Lấy sample shape từ R5.2
sample_data = np.load(f"{TENSOR_DIR_R52_218}/dev_r52_chunk_0.npz")
_, T_sess, F_sess = sample_data['X_sess'].shape
del sample_data; gc.collect()

# ==============================================================================
# 🔄 KHÔI PHỤC USER MAPPING CHỈ CHO R5.2
# ==============================================================================
print("🔄 Đang khôi phục User Mapping R5.2 (Chia 80/20 lấy Dev/Test)...")
...
def build_user_encoder(F_user, learning_rate=0.0001):
    ...
    x = Dense(32, name='User_Value_Projection')(x)
...
# ==============================================================================
# 🌀 PIPELINE HUẤN LUYỆN: TRAIN R5.2 | VAL R5.2 | TEST R5.2
# ==============================================================================
def train_single_fold_ablation(fold_idx, train_weeks, val_weeks, 
    ...
    user_encoder = build_user_encoder(F_user, learning_rate=0.0001)
...
if __name__ == '__main__':
    ...
    print("\n" + "="*75)
    print("🚀 PIPELINE BASELINE (DL): TRAIN R5.2 | VAL R5.2 | TEST R5.2")
    print("="*75)
    ...
    X_dev_52, y_dev_52_full, uid_dev_52, w_dev_52, day_dev_52, _, _ = load_pass1_session(TENSOR_DIR_R52_218, "dev_r52")
    _, y_r52_full, _, _, _, _, chunk_files_r52_test = load_pass1_session(TENSOR_DIR_R52_218, "test_r52")
    ...
    # VÌ CHỈ CÓ R5.2 NÊN OFFSET LÀ 0
    OFFSET = 0 
    print(f" -> Tập Train/Val chỉ dùng R5.2 (Không Offset).")

    # TẬP DỮ LIỆU GỐC BÂY GIỜ CHỈ LÀ R5.2
    ...
    TMP_RESULT_DIR = "./temp_results_dl_baseline_r52"
    ...
    # ==============================================================================
    OFFLINE_SAVE_PATH = '/kaggle/working/pipeline_dl_baseline_r52_only.npz'
    np.savez_compressed(
        OFFLINE_SAVE_PATH,
    ...
    print("\n" + "="*75)
    print(f"🏁 TOÀN BỘ PIPELINE BASELINE (DL TEST R5.2 ONLY) HOÀN TẤT")
    print("="*75)