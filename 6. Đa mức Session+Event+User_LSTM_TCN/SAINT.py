...
from tensorflow.keras.layers import MultiHeadAttention, LayerNormalization, Dense, Dropout, Permute, MaxPooling1D
...
vocab_sizes = {'act': 8, 'file_type': 7, 'http_type': 8, 'time': 5}
...
_, T_ev, F_ev = sample_data['X_ev'].shape
...
def load_pass2_event_selected(files, keep_indices, counts):
    X_e, curr = [], 0
    for i, f in enumerate(files):
        c = counts[i]
        if c == 0: continue
        end = curr + c
        mask = (keep_indices >= curr) & (keep_indices < end)
        local_idx = keep_indices[mask] - curr
        if len(local_idx) > 0:
            with np.load(f) as data: X_e.append(data['X_ev'][local_idx])
        curr = end
    return np.concatenate(X_e)
...
def saint_block(inputs, T_dim, F_feat, num_heads=4, ff_dim=256, dropout=0.2, name_prefix="SAINT"):
    attn_time = MultiHeadAttention(num_heads=num_heads, key_dim=64, dropout=dropout, name=f"{name_prefix}_Time_MHA")(inputs, inputs)
    ...
    ff_time = Dense(F_feat, name=f"{name_prefix}_Time_FF2")(ff_time) # Khôi phục chiều Feature
    ...
    attn_feat = MultiHeadAttention(num_heads=num_heads, key_dim=64, dropout=dropout, name=f"{name_prefix}_Feat_MHA")(x_feat, x_feat)
...
def build_event_encoder(T_ev, F_ev, learning_rate=0.0001):
    inp = Input(shape=(T_ev, F_ev), name='Input_Event')
    cont_feats = inp[:, :, :-4]; cat_act = inp[:, :, -4]; cat_file = inp[:, :, -3]; cat_http = inp[:, :, -2]; cat_time = inp[:, :, -1]     
    emb_act = Embedding(input_dim=vocab_sizes['act'], output_dim=16)(cat_act)
    emb_file = Embedding(input_dim=vocab_sizes['file_type'], output_dim=8)(cat_file)
    emb_http = Embedding(input_dim=vocab_sizes['http_type'], output_dim=8)(cat_http)
    emb_time = Embedding(input_dim=vocab_sizes['time'], output_dim=8)(cat_time)
    x = Concatenate(axis=-1)([cont_feats, emb_act, emb_file, emb_http, emb_time])
    
    positions = tf.range(start=0, limit=T_ev, delta=1)
    x = Dense(128, name='Ev_Feature_Projection')(x)
    pos_emb = Embedding(input_dim=T_ev, output_dim=128, name='Event_PosEmb')(positions)
    x = x + pos_emb
    x = saint_block(x, T_dim=T_ev, F_feat=128, num_heads=2, ff_dim=128, dropout=0.2, name_prefix="Ev_SAINT_1")
    x = GatedTemporalAttention(name='Event_Temporal_Attention')(x)
    latent = Dense(128, activation='relu', name='Event_Latent')(x)
    latent_bn = BatchNormalization(name='Event_Latent_BN')(latent) 
...
def build_session_encoder(T_sess, F_sess, learning_rate=0.0001):
    ...
    x = x + pos_emb
    x = saint_block(x, T_dim=T_sess, F_feat=128, num_heads=4, ff_dim=256, dropout=0.2, name_prefix="Sess_SAINT_1")
...
def build_user_encoder(F_user, learning_rate=0.0001):
    ...
    x = saint_block(x, T_dim=F_user, F_feat=x.shape[-1], num_heads=2, ff_dim=64, dropout=0.2,name_prefix="User_SAINT_1")
...
def build_score_level_fusion(fused_dim=12):
    inp = Input(shape=(fused_dim,), name='Input_12D_Scores')
    ...
    model = Model(inputs=inp, outputs=out, name="Standard_NN_Fusion")
...
def train_single_fold(fold_idx, train_weeks, val_weeks, X_dev_sess_full, y_dev_full, uid_dev_full, w_dev_full, day_dev_full, chunk_files, chunk_counts, user_lookup_dev, F_user, user_lookup_test, chunk_files_test, len_y_test):
    ...
    X_train_ev = load_pass2_event_selected(chunk_files, keep_global_idx, chunk_counts)
    
    X_val_sess = X_dev_sess_full[idx_val]
    y_val = y_dev_full[idx_val]
    uid_val = uid_dev_full[idx_val]
    day_val = day_dev_full[idx_val]
    X_val_ev = load_pass2_event_selected(chunk_files, idx_val, chunk_counts)
    ...
    np.nan_to_num(X_train_ev, nan=0.0, posinf=65000.0, neginf=-65000.0, copy=False)
    np.nan_to_num(X_val_ev, nan=0.0, posinf=65000.0, neginf=-65000.0, copy=False)
    
    train_ev_cont = X_train_ev[:, :, :-4].astype(np.float32, copy=False)
    train_ev_cat  = X_train_ev[:, :, -4:] 
    val_ev_cont = X_val_ev[:, :, :-4].astype(np.float32, copy=False)
    val_ev_cat  = X_val_ev[:, :, -4:]
    del X_train_ev, X_val_ev; gc.collect()
    
    F_ev_cont = train_ev_cont.shape[-1]
    scaler_ev = StandardScaler(copy=False)
    train_ev_cont = scaler_ev.fit_transform(train_ev_cont.reshape(-1, F_ev_cont)).astype(np.float32, copy=False).reshape(-1, T_ev, F_ev_cont)
    val_ev_cont = scaler_ev.transform(val_ev_cont.reshape(-1, F_ev_cont)).astype(np.float32, copy=False).reshape(-1, T_ev, F_ev_cont)
    ...
    event_encoder = build_event_encoder(T_ev, F_ev, learning_rate=0.0001)
    session_encoder = build_session_encoder(T_sess, F_sess, learning_rate=0.0001)
    user_encoder = build_user_encoder(F_user)
    
    es_ev = EarlyStopping(monitor='val_loss', patience=6, restore_best_weights=True)
    ...
    lr_ev = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=3, min_lr=1e-6, verbose=1)
    ...
    start_ev = time.time()
    gen_train_ev = BalancedUniversalGenerator(train_ev_cont, y_train_ohe, X_cat=train_ev_cat, batch_size=128, hacker_ratio=0.25)
    gen_val_ev = BalancedUniversalGenerator(val_ev_cont, y_val_ohe, X_cat=val_ev_cat, shuffle=False, batch_size=128, hacker_ratio=0)
    event_encoder.fit(gen_train_ev, validation_data=gen_val_ev, epochs=100, callbacks=[es_ev, lr_ev], verbose=0)
    time_ev = time.time() - start_ev
    print(f"Xong Event Encoder sau: {time_ev/60:.2f} phút")
    del gen_train_ev, gen_val_ev; gc.collect()

    start_sess = time.time()
    gen_train_sess = BalancedUniversalGenerator(X_train_sess, y_train_ohe, hacker_ratio=0.25)
    gen_val_sess = BalancedUniversalGenerator(X_val_sess, y_val_ohe, shuffle=False, hacker_ratio=0)
    ...
    gen_val_user = BalancedUniversalGenerator(X_val_user, y_val_ohe, shuffle=False, hacker_ratio=0)
    ...
    total_enc_time = (time_ev + time_sess + time_user) / 60
    print(f"TỔNG THỜI GIAN TRAIN 3 ENCODER FOLD {fold_idx + 1}: {total_enc_time:.2f} phút")
    
    # 8. RÚT XÁC SUẤT
    extract_ev = event_encoder 
    ...
    prob_train_ev = predict_eager_batch(extract_ev, train_ev_cont, X_cat=train_ev_cat, batch_size=64)
    del train_ev_cont, train_ev_cat; gc.collect() 
    ...
    train_scores_raw = np.hstack([prob_train_ev, prob_train_sess, prob_train_user]).astype(np.float32, copy=False)
    del prob_train_ev, prob_train_sess, prob_train_user; gc.collect() 

    prob_val_ev = predict_eager_batch(extract_ev, val_ev_cont, X_cat=val_ev_cat, batch_size=64)
    del val_ev_cont, val_ev_cat; gc.collect()
    ...
    val_scores_raw = np.hstack([prob_val_ev, prob_val_sess, prob_val_user]).astype(np.float32, copy=False)
    del prob_val_ev, prob_val_sess, prob_val_user; gc.collect()
    ...
    # 9. HUẤN LUYỆN NEURAL NETWORK CLASSIFIER 
    classifier = build_score_level_fusion(fused_dim=12) 
    ...
    # LƯU LẠI LỊCH SỬ HUẤN LUYỆN CỦA CLASSIFIER
    ...
    prob_test_sess_list, prob_test_ev_list, prob_test_user_list = [], [], []
    for f in chunk_files_test: 
        with np.load(f) as data:
            if len(data['y']) == 0: continue
            batch_sess, batch_ev = data['X_sess'], data['X_ev']
            batch_uid, batch_day = data['user_id'], data['day']
        ...
        batch_ev = np.nan_to_num(batch_ev, nan=0.0, posinf=65000.0, neginf=-65000.0, copy=False)
        batch_ev_cont = batch_ev[:, :, :-4].astype(np.float32, copy=False)
        batch_ev_cat  = batch_ev[:, :, -4:]
        batch_ev_cont_s = scaler_ev.transform(batch_ev_cont.reshape(-1, F_ev_cont)).astype(np.float32, copy=False).reshape(-1, T_ev, F_ev_cont)
        batch_ev_s = np.concatenate([batch_ev_cont_s, batch_ev_cat], axis=-1)
        prob_test_ev_list.append(predict_eager_batch(extract_ev, batch_ev_s, batch_size=64))
    ...
    prob_test_ev = np.concatenate(prob_test_ev_list, axis=0)
    prob_test_sess = np.concatenate(prob_test_sess_list, axis=0)
    prob_test_user = np.concatenate(prob_test_user_list, axis=0)
    
    test_scores_raw = np.hstack([prob_test_ev, prob_test_sess, prob_test_user]).astype(np.float32, copy=False)
    del prob_test_sess_list, prob_test_ev_list, prob_test_user_list, prob_test_ev, prob_test_sess, prob_test_user; gc.collect()
...
if __name__ == '__main__':
    ...
    for fold_idx in range(len(time_folds)):
        ...
        # Đọc lịch sử huấn luyện để lát vẽ biểu đồ
    ...
    # 1. Lưu các mảng Numpy (Xác suất, Nhãn, AUC)
    np.savez(
        '/kaggle/working/tri_view_predictions.npz',
    ...
    with open('/kaggle/working/tri_view_histories.pkl', 'wb') as f:
        pickle.dump(training_histories, f)
        
    print("Đã lưu xong 2 file: tri_view_predictions.npz và tri_view_histories.pkl!")
    print("="*75 + "\n")
    ...
    def plot_confusion_matrix(cm, classes, title='Confusion Matrix - Tập Test'):
    ...
    plot_confusion_matrix(conf_matrix_test, classes_names, title="Ma Trận Nhầm Lẫn")

    # 2. BIỂU ĐỒ LOSS & ACCURACY (TRAINING HISTORY)
    def plot_training_history_average(histories):
        ...
        for key in histories[0].keys():
            ...
            for h in histories:
                arr = h[key]
                # Tính số lượng epoch bị thiếu so với max_epochs
                pad_length = max_epochs - len(arr)
                # mode='edge': Lấy giá trị cuối cùng đắp vào phần bị thiếu
        ...
        #axes[0].set_title(f'Trung bình Loss qua {len(histories)} Folds', fontweight='bold')
        ...
        if 'accuracy' in avg_history:
            ...
            #axes[1].set_title(f'Trung bình Accuracy qua {len(histories)} Folds', fontweight='bold')
    ...
    # 3. BIỂU ĐỒ ROC CURVE CHO TỪNG KỊCH BẢN (One-vs-Rest)
    def plot_multiclass_roc(y_test, y_probs, n_classes, classes_names):
        ...
        for i in range(n_classes):
            ...
            roc_auc = auc(fpr, tpr)
            # Chỉ vẽ đường ROC nếu có dữ liệu để tính
        ...
        plt.title('Biểu đồ ROC Đa lớp', fontsize=15, fontweight='bold', pad=15)
