...
def load_userday_lookup(file_path, user_mapping):
    ...
    feature_cols = [c for c in df.columns if c not in ['user', 'insider', 'user_id']]
...
def pool_3d_to_2d(X_3d):
    ...
    return np.hstack([X_mean, X_max]).astype(np.float32)
...
def train_single_fold_lgbm(fold_idx, train_weeks, val_weeks, X_dev_sess_full, y_dev_full, uid_dev_full, w_dev_full, day_dev_full, chunk_files, chunk_counts, user_lookup_dev, F_user, user_lookup_test, chunk_files_test, len_y_test):
    ...
    # 4. Statistical Pooling & Concatenation
    X_train_concat = np.hstack([pool_3d_to_2d(X_train_ev), pool_3d_to_2d(X_train_sess), X_train_user])
    X_val_concat = np.hstack([pool_3d_to_2d(X_val_ev), pool_3d_to_2d(X_val_sess), X_val_user])
    
    del X_train_ev, X_train_sess, X_val_ev, X_val_sess; gc.collect()
    ...
    for f in chunk_files_test: 
        with np.load(f) as data:
            if len(data['y']) == 0: continue
            batch_sess, batch_ev, batch_uid = data['X_sess'], data['X_ev'], data['user_id']
            batch_concat = np.hstack([
                pool_3d_to_2d(batch_ev), pool_3d_to_2d(batch_sess), 
    ...
    # 8. Lưu kết quả tạm thời của từng Fold
    np.savez(
...
if __name__ == '__main__':
    ...
    np.savez(
        '/kaggle/working/tri_view_predictions_lgbm.npz',
    ...
    with open('/kaggle/working/tri_view_histories_lgbm.pkl', 'wb') as f:
        pickle.dump(training_histories, f)
        
    print("Đã lưu xong 2 file: tri_view_predictions_lgbm.npz và tri_view_histories_lgbm.pkl!")
    print("="*75 + "\n")
    ...
    def plot_training_history_average(histories):
        ...
        #axes[0].set_title(f'Trung bình Loss (LightGBM - {len(histories)} Folds)', fontweight='bold')
        ...
        #axes[1].set_title(f'Trung bình Accuracy (LightGBM - {len(histories)} Folds)', fontweight='bold')
    ...
    plt.title("Confusion Matrix - LightGBM (Tri-View)", fontsize=14, fontweight='bold', pad=15)
    ...
    plt.title(f'Biểu đồ ROC Đa lớp - LightGBM (Macro AUC = {test_auc:.4f})', fontsize=15, fontweight='bold', pad=15)
