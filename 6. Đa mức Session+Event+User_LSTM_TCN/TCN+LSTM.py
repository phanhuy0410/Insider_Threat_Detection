...
def build_event_encoder(T_ev, F_ev, learning_rate=0.0001):
    ...
    for d in dilations:
        x = tcn_residual_block(x, n_filters=128, kernel_size=3, dilation_rate=d, dropout_rate=0.2, block_name=f"Ev_TCN_D{d}")

    x = LSTM(128, return_sequences=True, name='Event_LSTM')(x)
    x = Dropout(0.2)(x)
...
def build_session_encoder(T_sess, F_sess, learning_rate=0.0001):
    ...
    x = tcn_residual_block(inp, n_filters=64, kernel_size=2, dilation_rate=1, dropout_rate=0.2, block_name="Sess_TCN_D1")
    x = tcn_residual_block(x, n_filters=64, kernel_size=2, dilation_rate=2, dropout_rate=0.2, block_name="Sess_TCN_D2")
    x = LSTM(128, return_sequences=True, name='Session_LSTM')(x)
    x = Dropout(0.2)(x)
...
def build_user_encoder(F_user, learning_rate=0.0001):
    ...
    x = tcn_residual_block(x, n_filters=32, kernel_size=3, dilation_rate=2, dropout_rate=0.2, block_name="User_TCN_D2")
    x = LSTM(32, return_sequences=True, name='User_LSTM')(x)
    x = Dropout(0.2)(x)
...
def train_single_fold(fold_idx, train_weeks, val_weeks, X_dev_sess_full, y_dev_full, uid_dev_full, w_dev_full, day_dev_full, chunk_files, chunk_counts, user_lookup_dev, F_user, user_lookup_test, chunk_files_test, len_y_test):
    ...
    start_ev = time.time()
    gen_train_ev = BalancedUniversalGenerator(train_ev_cont, y_train_ohe, X_cat=train_ev_cat, hacker_ratio=0.25)
    gen_val_ev = BalancedUniversalGenerator(val_ev_cont, y_val_ohe, X_cat=val_ev_cat, shuffle=False, hacker_ratio=0)
    ...
    prob_train_ev = predict_eager_batch(extract_ev, train_ev_cont, X_cat=train_ev_cat, batch_size=512)
    ...
    prob_val_ev = predict_eager_batch(extract_ev, val_ev_cont, X_cat=val_ev_cat, batch_size=512)
    ...
    for f in chunk_files_test: 
        ...
        prob_test_ev_list.append(predict_eager_batch(extract_ev, batch_ev_s, batch_size=512))
...
if __name__ == '__main__':
    ...
    plot_confusion_matrix(conf_matrix_test, classes_names, title="Ma Trận Nhầm Lẫn (Non-Fuzzy Neural Network)")
