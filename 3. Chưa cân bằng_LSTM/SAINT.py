...
from tensorflow.keras.layers import Input, GRU, LSTM, Bidirectional, Dropout, Dense, BatchNormalization
...
def build_my_model(T, F):
    ...
    attn_time = MultiHeadAttention(
        num_heads=4,
        key_dim=32,
        dropout=0.1
    )(inputs, inputs)

    x_time = LayerNormalization(epsilon=1e-6)(
        inputs + attn_time
    )

    ff_time = Dense(128, activation='gelu')(x_time)
    ff_time = Dropout(0.1)(ff_time)
    ff_time = Dense(F)(ff_time)
    ...
    # 2. FEATURE ATTENTION
    # =====================================================
    x_feat = Permute((2, 1))(x_time)

    x_feat = Dense(64)(x_feat)

    attn_feat = MultiHeadAttention(
        num_heads=4,
        key_dim=16,
        dropout=0.1
    )(x_feat, x_feat)

    x_feat = LayerNormalization(epsilon=1e-6)(
        x_feat + attn_feat
    )

    ff_feat = Dense(128, activation='gelu')(x_feat)
    ff_feat = Dropout(0.1)(ff_feat)
    ff_feat = Dense(64)(ff_feat)
    ...
    # 3. POOLING
    # =====================================================
    x = GlobalAveragePooling1D()(x_feat)
...
plt.title("Confusion Matrix - SAINT", fontsize=14, fontweight='bold', pad=15)
...
axes[0].set_title('Biểu đồ Loss', fontweight='bold')
...
axes[1].set_title('Biểu đồ Accuracy', fontweight='bold')
...
plt.title(f'Biểu đồ Đa lớp ROC Curve', fontsize=15, fontweight='bold', pad=15)
