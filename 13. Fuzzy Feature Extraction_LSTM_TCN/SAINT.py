...
def build_session_encoder(T_sess, F_sess, flat_fuzzy_indices, centers_init, sigmas_init, learning_rate=0.0001):
    ...
    fuzzy_output_dim = len(flat_fuzzy_indices) * 3
    x = Dense(fuzzy_output_dim, activation='relu', name='Session_Rule_Inference')(x)
    ...
    x = saint_block(x, T_dim=T_sess, F_feat=128, num_heads=4, key_dim=32, ff_dim=256, dropout=0.2, name_prefix="Sess_SAINT_1")
...
def build_user_encoder(F_user, flat_fuzzy_indices, centers_init, sigmas_init, learning_rate=0.0001):
    ...
    x = FeatureTokenEmbedding(d_model=32, name='User_Feature_Tokens')(x)
    T_user = int(x.shape[1])
    x = saint_block(x, T_dim=T_user, F_feat=32, num_heads=4, key_dim=8, ff_dim=64, dropout=0.2, name_prefix="User_SAINT_1")
